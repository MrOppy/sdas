  ...
  useEffect(() => {
    const observerCallback = (entries) => {
      entries.forEach((entry) => {
        ...
        const visibleHeight = Math.min(targetRect.bottom, windowHeight) - Math.max(targetRect.top, 0);
        const sectionHeight = targetRect.height;
        const visibilityRatio = visibleHeight / sectionHeight;

        // Update active section when more than 40% of the section is visible
    ...
    const observer = new IntersectionObserver(observerCallback, {
      threshold: Array.from({ length: 41 }, (_, i) => i * 0.025), // Create thresholds from 0 to 1 in steps of 0.025
    });
